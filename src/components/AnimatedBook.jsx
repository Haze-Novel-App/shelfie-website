...
//       // Title - Adjusted Y to be higher (Task 1)
//       ctx.textAlign = 'center'; ctx.fillStyle = '#1a1a1a';
//       ctx.font = 'italic 700 70px "Playfair Display", serif';
//       ctx.fillText(data.title, 512, 180); 

//       // Gold Line
//       ctx.strokeStyle = '#d4af37'; ctx.lineWidth = 3;
//       ctx.beginPath(); ctx.moveTo(300, 220); ctx.lineTo(724, 220); ctx.stroke();

//       // Body Text - Task 4 (Strict margins)
//       ctx.font = '400 34px "Inter", sans-serif';
//       ctx.fillStyle = '#333333';
//       const words = data.body.split(' ');
//       let line = '', y = 300, lineHeight = 48, maxTextWidth = 800;
...
//       // Buttons - Task 4 (Clamped Y)
//       if (data.showButtons) {
//         const btnY = Math.min(y + 60, 1200); 
//         const imgApp = new Image();
//         imgApp.crossOrigin = "Anonymous";
//         imgApp.src = "https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg";
//         imgApp.onload = () => { ctx.drawImage(imgApp, 222, btnY, 260, 78); tex.needsUpdate = true; };
...
//     CONTENT.forEach((data, i) => {
//       const pivot = new THREE.Group();
//       pivot.position.set(-BOOK_WIDTH / 2, 0, 0.2 - (i * 0.03)); // Slight Z offset for layering
      
//       const textTex = createTextTexture(data);
//       const textMat = new THREE.MeshStandardMaterial({ map: textTex, roughness: 0.8 });
//       const imgMat = new THREE.MeshStandardMaterial({ color: 0xffffff });

//       // Only load image for the BACK of the page if it exists
//       if (data.img) {
//         loader.load(data.img, (t) => { 
//             imgMat.map = t; 
//             imgMat.needsUpdate = true; 
//         });
//       }

//       // Material Index: 4 = Front (Text), 5 = Back (Image)
//       const mesh = new THREE.Mesh(new THREE.BoxGeometry(BOOK_WIDTH, BOOK_HEIGHT, 0.01), [paperMat, paperMat, paperMat, paperMat, textMat, imgMat]);
//       mesh.position.x = BOOK_WIDTH / 2;
//       mesh.userData = { originalVertices: mesh.geometry.attributes.position.array.slice() };
//       pivot.add(mesh);
//       bookGroup.add(pivot);
...
//       // Page Rotation Logic
//       const START_R = 0.15, START_C = 0.85;
//       let cTarg = (p > START_R) ? -Math.PI * 0.98 : 0;
//       frontCoverPivot.rotation.y += (cTarg - frontCoverPivot.rotation.y) * 0.08;

//       pageMeshes.forEach((mesh, i) => {
//         const pivot = mesh.parent;
//         const s = 0.2 + (i * 0.15), e = 0.35 + (i * 0.15);
//         let localP = (p > s) ? Math.min((p - s) / (e - s), 1) : 0;
        
//         // Don't turn the very last page too early
//         if (p > START_C) localP = 0; 
...
//         const pos = mesh.geometry.attributes.position;
//         const orig = mesh.userData.originalVertices;
//         const bend = Math.sin(Math.abs(pivot.rotation.y / Math.PI) * Math.PI) * 0.4;
//         for (let k = 0; k < pos.count; k++) {
//           const dist = (orig[k * 3] + BOOK_WIDTH / 2) / BOOK_WIDTH;
//           pos.setZ(k, orig[k * 3 + 2] + Math.sin(dist * Math.PI) * bend);
//         }
...
//       // Camera & Book Positioning - Task 2 (Lowering the book when open)
//       let tX = 0, tZ = 16, tRotY = 0, tY = 0.5; 
      
//       if (p < 0.15) { 
//         tX = 4.5; tZ = 16; tRotY = -0.4; tY = 0.6; // Initial position (Task 1: higher for cover)
//       } else if (p > 0.85) { 
//         tX = -2.5; tZ = 14; tRotY = 0.5; tY = -0.2; // Registration phase
//       } else { 
//         tX = 1.0; tZ = 10; tRotY = 0; tY = -0.8; // Open book phase - LOWERED to avoid Nav (Task 2)
//       }
...
//           <h1 className="hero-title">Stories were never meant <br /> to be one-way</h1>
//           <p className="hero-desc">On Shelfie, reading is just the beginning. Discover writers, explore stories, and interact directly with authors.</p>
//           <button className="btn-primary">Get Started on Shelfie</button>
//         </div>
...
const AnimationBook = () => {
 ...
 useEffect(() => {
    ...
    const SPACING = 0.04; // Tightened spacing
    const SPINE_RADIUS = 0.12; // DECREASED WIDTH HERE (Original was 0.25)
    ...
    // Spine - Adjusted geometry and position
